...
SAP.common.defineNS("SAP.CR.Viewer.Instance", function(viewerName, parentID) {
    ...
    this.setFreezePanesSize = function(width, height) {
        if (!bobj.isNumber(width))
            throw SAP.CR.Viewer.Exceptions.IllegalArgumentException.create(L_bobj_crv_API_InvalidFreezePanesSize.replace("{0}", width).replace(
                    "{1}", "width"));
        if (!bobj.isNumber(height))
            throw SAP.CR.Viewer.Exceptions.IllegalArgumentException.create(L_bobj_crv_API_InvalidFreezePanesSize.replace("{0}", height).replace(
                    "{1}", "height"));
        
        if (isQueueEvents())
            pendingEvents.push(SAP.CR.Events.SetFreezePanesSize(width, height));
        else
            bobj.event.publish('setFreezePanesSize', name, width, height);
    };
    ...
    this.setToolPanelViewMode = function(viewMode) {
        if (!isInitialized) {
            if (isValidToolPanelViewMode(viewMode))
                pendingEvents.push(SAP.CR.Events.SetToolPanelViewMode(viewMode));
            else
                throw SAP.CR.Viewer.Exceptions.IllegalArgumentException.create(L_bobj_crv_API_InvalidToolPanelViewMode.replace("{0}", viewMode).replace(
                        "{1}", "SAP.CR.Viewer.ToolPanelViewMode"));
        } else
            throw SAP.CR.Viewer.Exceptions.IllegalOperationException.create(L_bobj_crv_API_InvalidFunctionAfterInit.replace("{0}", "SAP.CR.Viewer.Instance.setToolPanelViewMode"));
    };
    
    function isValidToolPanelViewMode(viewMode) {
        if (!viewMode)
            return false;

        for (var key in SAP.CR.Viewer.ToolPanelViewMode) {
            if (SAP.CR.Viewer.ToolPanelViewMode[key] == viewMode)
                return true;
        }
    ...
    this.setHasToggleGroupTreeButton = function(hasButton) {
        if (!bobj.isBoolean(hasButton))
            throw SAP.CR.Viewer.Exceptions.IllegalArgumentException.create(L_bobj_crv_API_InvalidBoolean.replace("{0}", hasButton));

        if (!isInitialized)
            pendingEvents.push(SAP.CR.Events.SetHasToggleGroupTreeButton(hasButton));
        else
            throw SAP.CR.Viewer.Exceptions.IllegalOperationException.create(L_bobj_crv_API_InvalidFunctionAfterInit.replace("{0}", "SAP.CR.Viewer.Instance.setHasToggleGroupTreeButton"));
    };

    this.setHasToggleParameterPanelButton = function(hasButton) {
        if (!bobj.isBoolean(hasButton))
            throw SAP.CR.Viewer.Exceptions.IllegalArgumentException.create(L_bobj_crv_API_InvalidBoolean.replace("{0}", hasButton));

        if (!isInitialized)
            pendingEvents.push(SAP.CR.Events.SetHasToggleParameterPanelButton(hasButton));
        else
            throw SAP.CR.Viewer.Exceptions.IllegalOperationException.create(L_bobj_crv_API_InvalidFunctionAfterInit.replace("{0}", "SAP.CR.Viewer.Instance.setHasToggleParameterPanelButton"));
    };

    this.setHasSearchButton = function(hasButton) {
        if (!bobj.isBoolean(hasButton))
            throw SAP.CR.Viewer.Exceptions.IllegalArgumentException.create(L_bobj_crv_API_InvalidBoolean.replace("{0}", hasButton));

        if (!isInitialized)
            pendingEvents.push(SAP.CR.Events.SetHasSearchButton(hasButton));
        else
            throw SAP.CR.Viewer.Exceptions.IllegalOperationException.create(L_bobj_crv_API_InvalidFunctionAfterInit.replace("{0}", "SAP.CR.Viewer.Instance.setHasSearchButton"));
    };
    ...
    this.setEnableDrillDown = function(enable) {
        if (!bobj.isBoolean(enable))
            throw SAP.CR.Viewer.Exceptions.IllegalArgumentException.create(L_bobj_crv_API_InvalidBoolean.replace("{0}", enable));

        if (!isInitialized)
            pendingEvents.push(SAP.CR.Events.SetEnableDrillDown(enable));
        else
            throw SAP.CR.Viewer.Exceptions.IllegalOperationException.create(L_bobj_crv_API_InvalidFunctionAfterInit.replace("{0}", "SAP.CR.Viewer.Instance.setEnableDrillDown"));
    };
...
SAP.common.defineNS("SAP.CR.Viewer.ToolPanelViewMode", {
    NONE : bobj.crv.ToolPanelType.None,
    GROUPTREE : bobj.crv.ToolPanelType.GroupTree,
    PARAMETER : bobj.crv.ToolPanelType.ParameterPanel,
    SEARCH : bobj.crv.ToolPanelType.Search
});
