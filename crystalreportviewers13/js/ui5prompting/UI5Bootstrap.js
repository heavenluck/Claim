...
if (typeof (BILaunchpadApp) == 'undefined') {
    var srcLoc;
    if (bobj.nodeResources) {
        srcLoc = bobj.nodeResources;
    } else {
        srcLoc = location.pathname.substr(0, location.pathname.indexOf('CrystalReports')) + 'PlatformServices/sapui5/resources/sap-ui-core.js';
        var request = new XMLHttpRequest();
        request.open('HEAD', srcLoc, false);
        request.onreadystatechange = function(){
            if (request.readyState === 4) {
                if (request.status === 404) {
                    srcLoc = location.pathname.substr(0, location.pathname.indexOf('CrystalReports')) + 'CrystalReports/resources/sap-ui-core.js';
                }
            }
        };
        request.send();
